...
public class RedisConfiguration {
    ...
    @Bean
    public RedisTemplate<String, Produto> redisTemplate(RedisConnectionFactory factory) {
        RedisTemplate<String, Produto> template = new RedisTemplate<>();
        template.setConnectionFactory(factory);
        template.setKeySerializer(new StringRedisSerializer());
        template.setValueSerializer(new Jackson2JsonRedisSerializer<>(Produto.class));
        return template;
    }
